...
class LoginServiceImplTest {
    ...
    class LoginTests {
        ...
        void login_validCredentials_success() {
            ...
            when(tokenUtil.generateAccessToken(anyString(), any(Role.class), anyString())).thenReturn("access-token");
            when(tokenUtil.generateRefreshToken(anyString(), any(Role.class), anyString())).thenReturn("refresh-token");
    ...
    class RefreshTokenTests {
        ...
        void refreshToken_validToken_success() {
            ...
            when(tokenUtil.generateAccessToken(anyString(), any(Role.class), anyString())).thenReturn("new-access-token");
            when(tokenUtil.generateRefreshToken(anyString(), any(Role.class), anyString())).thenReturn("new-refresh-token");
            when(authRepository.save(any(Auth.class))).thenReturn(auth);
    ...
    class IntegrationScenarioTests {
        ...
        void scenario_adminLogin() {
            ...
            when(passwordEncoder.matches(password, "hashedPassword")).thenReturn(true);
            when(tokenUtil.generateAccessToken(eq("admin-id"), eq(Role.ADMIN), anyString())).thenReturn("admin-access-token");
            when(tokenUtil.generateRefreshToken(eq("admin-id"), eq(Role.ADMIN), anyString())).thenReturn("admin-refresh-token");
