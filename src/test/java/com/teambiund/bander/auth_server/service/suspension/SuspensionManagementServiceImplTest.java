...
class SuspensionManagementServiceImplTest {
  ...
  @InjectMocks private SuspensionManagementServiceImpl suspensionService;
  ...
  class ReleaseTests {
    ...
    void release_suspendedUser_success() {
      ...
      Auth auth =
          Auth.builder().id(userId).email("test@example.com").status(Status.SUSPENDED).build();
    ...
    void release_blockedUser_success() {
      ...
      Auth auth =
          Auth.builder().id(userId).email("test@example.com").status(Status.BLOCKED).build();
    ...
    void release_variousStatuses() {
      // given - SLEEPING 상태
      Auth sleepingAuth = Auth.builder().id("sleeping-user").status(Status.SLEEPING).build();
  ...
  class SuspendTests {
    ...
    void suspend_bidirectionalRelationship() {
      ...
      Auth user =
          Auth.builder().id(userId).status(Status.ACTIVE).suspensions(new ArrayList<>()).build();

      Auth admin = Auth.builder().id(suspenderUserId).userRole(Role.ADMIN).build();
    ...
    void suspend_suspenderNotFound_throwsException() {
      ...
      Auth user = Auth.builder().id(userId).status(Status.ACTIVE).build();
  ...
  class ExceptionTests {
    ...
    void release_nullUserId_throwsException() {
      // when & then
      assertThatThrownBy(() -> suspensionService.release(null)).isInstanceOf(Exception.class);
    }
  ...
  class AuthorizationTests {
    ...
    void authorization_onlyAdminCanSuspend() {
      ...
      Auth regularUser = Auth.builder().id(suspenderUserId).userRole(Role.USER).build();
      ...
      // given - GUEST 역할
      Auth guest = Auth.builder().id(suspenderUserId).userRole(Role.GUEST).build();
    ...
    void authorization_adminRoleCheck() {
      ...
      Auth admin = Auth.builder().id(adminUserId).userRole(Role.ADMIN).build();
