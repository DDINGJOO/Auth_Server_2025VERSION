...
/**
 * Integration tests to ensure plaintext email inputs are correctly matched
 * against encrypted-at-rest emails stored in the database.
 */
...
public class EmailEncryptionAtRestIntegrationTest {
    ...
    @Autowired
    private WithdrawalManagementServiceImpl withdrawalManagementService;

    @Autowired
    private CipherStrategy aesCipherStrategy; // bean name: aesCipherStrategy

    @Autowired
    private CipherStrategy pbkdf2CipherStrategy; // bean name: pbkdf2CipherStrategy


    @MockBean
    private KeyProvider keyProvider; // 생성되는 키는 고정값으로

    @MockBean
    private TokenUtil tokenUtil; // 토큰 생성은 단순 목으로 대체
