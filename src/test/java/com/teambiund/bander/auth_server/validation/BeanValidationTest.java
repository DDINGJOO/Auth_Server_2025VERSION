...
class BeanValidationTest {
  ...
  void signupRequest_BlankEmail() {
    ...
    assertThat(violations.stream().anyMatch(v -> v.getMessage().equals("이메일은 필수 입력 항목입니다")))
        .isTrue();
  }
  ...
  void signupRequest_NullConsent() {
    ...
    assertThat(violations.stream().anyMatch(v -> v.getMessage().equals("동의 항목은 필수입니다"))).isTrue();
  }
  ...
  void consentRequest_BlankConsentId() {
    ...
    // Then
    assertThat(violations).hasSize(1);
    assertThat(violations.iterator().next().getMessage()).isEqualTo("동의 항목 ID는 필수입니다");
  }
  ...
  void suspendRequest_BlankReason() {
    ...
    // Then
    assertThat(violations).hasSize(1);
    assertThat(violations.iterator().next().getMessage()).isEqualTo("정지 사유는 필수입니다");
  }
  ...
  void suspendRequest_InvalidSuspendDay() {
    ...
    // Then
    assertThat(violations).hasSize(1);
    assertThat(violations.iterator().next().getMessage()).isEqualTo("정지 일수는 최소 1일 이상이어야 합니다");
  }
  ...
  void tokenRefreshRequest_BlankRefreshToken() {
    ...
    // Then
    assertThat(violations).hasSize(1);
    assertThat(violations.iterator().next().getMessage()).isEqualTo("리프레시 토큰은 필수입니다");
  }
  ...
  void tokenRefreshRequest_BlankDeviceId() {
    ...
    // Then
    assertThat(violations).hasSize(1);
    assertThat(violations.iterator().next().getMessage()).isEqualTo("디바이스 ID는 필수입니다");
  }
