...
public class Auth {
  ...
  /** History 추가 편의 메서드 양방향 연관관계 설정 */
  public void addHistory(History history) {
    this.history.add(history);
    history.setUser(this);
  }

  /** Consent 추가 편의 메서드 양방향 연관관계 설정 */
  ...
  /** Withdraw 설정 편의 메서드 양방향 연관관계 설정 */
  ...
  public void markAsDeleted(String withdrawReason) {
    ...
    Withdraw withdraw =
        Withdraw.builder().withdrawReason(withdrawReason).withdrawAt(LocalDateTime.now()).build();
  ...
  /**
   * ConsentsTable을 이용한 Consent 추가 헬퍼 메서드 Service 레이어에서 ID와 시간을 주입받아 사용
   *
   * @param consentId ID (Service에서 생성)
   * @param consentsTable 동의 유형 테이블
   * @param consentedAt 동의 시각 (Service에서 생성)
   */
  public void addConsentWithTable(
      String consentId, ConsentsTable consentsTable, LocalDateTime consentedAt) {
    Consent consent = Consent.builder().id(consentId).consentedAt(consentedAt).build();
    consent.setConsentsTable(consentsTable);
    this.addConsent(consent);
  }

  /** ConsentsTable 기준으로 Consent 제거 편의 메서드 특정 동의 유형의 모든 동의 기록을 제거 */
  public void removeConsentByTable(ConsentsTable consentsTable) {
    this.consent.removeIf(
        c ->
            c.getConsentsTable() != null
