...
import static com.teambiund.bander.auth_server.auth.util.data.ConsentTableInit.requiredConsents;
...
/**
 * 필수 동의항목 검증 구현체 - requiredConsents에 정의된 필수 항목들이 모두 동의되었는지 확인 - consentId로 조회하며, 버전 무관하게
 * consentName(consentType)만 검증
 */
public class RequiredConsentsValidator
    implements ConstraintValidator<RequiredConsents, List<ConsentRequest>> {
      ...
      if (!consentedNames.contains(required.getConsentName())) {
        ...
        context
            .buildConstraintViolationWithTemplate(
                String.format("필수 동의 항목 '%s'에 동의하지 않았습니다", required.getConsentName()))
