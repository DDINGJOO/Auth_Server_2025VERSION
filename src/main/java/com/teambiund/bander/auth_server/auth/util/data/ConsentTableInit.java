...
import com.teambiund.bander.auth_server.auth.repository.ConsentTableRepository;
...
/**
 * Consent 테이블 초기화 유틸리티. - DB에 존재하는 consents_name 레코드를 모두 읽어와 static Map(consentMaps)에 저장한다. - 같은
 * consentName이 여러 개 존재하면 version 필드(v1.1, v1.2 등)를 비교하여 최신 버전만 남긴다. - version 형식은 "v1.1" 또는 "1.1"
 * 형태를 가정하고, 접두사 'v'를 무시한 뒤 도트(.)로 구분된 숫자 파트를 좌측부터 비교한다.
 */
@Component
